...
var CACHE = 'tf-attend-v90'; // 08-16 그 '두 번째'가 실제로 앱을 닫게(첫 뒤로에 가드 재적재 안 함 — 실측) / v89 메인에서 뒤로=스와이프 두 번이라야 나감(§4.11-4 ex18) / v88 알약이 날짜 넘어가면 스스로 새 코드를 읽음(=배포가 도달함)+앱이 PIN 요구 이유를 로그에 남김 / v87 파란 상자 차단·겹친 낙관 쓰기 보존 / v86 퇴근 시각 24시간제 한 길
var SHELL = [
